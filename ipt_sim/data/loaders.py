import os, torch, numpy as np, pandas as pd, scipy.io as sio
...
class IptSimDataset(Dataset):
    def __init__(
        ...
        self.seed_idxs = seed_idxs if seed_idxs is not None else list(range(len(self.seed_labels)))
    ...
    def __getitem__(self, idx):
        item = self.seed_filelist[idx]
        return {"features": item['features'], "y": item["label"]}

    def __len__(self):
        return len(self.seed_filelist)
...
class InstrumentSplitGenerator(SplitGenerator):
    ...
    def __init__(self, seed_csv: str = './jasmp/seed_filelist.csv', overfit: bool = False):
...
class KFoldsSplitGenerator(SplitGenerator):
    ...
    def __init__(self, seed_csv: str = './jasmp/seed_filelist.csv', n_splits=4, overfit: bool = False):
        super().__init__(seed_csv)
