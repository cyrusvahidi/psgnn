...
from .audio_utils import resample_audio, normalize_audio
...
def get_sol_filepaths(sol_dir, filelist=None):
    ...
    '''
    filepaths = [os.path.relpath(f, sol_dir) 
...
def get_sol_files(df, data_dir):
    ...
    files = [os.path.join(data_dir, np_f) 
...
def create_numpy_files(arrays: Union[np.ndarray], 
                       fnames: Union[str], 
                       output_dir: str,
...
def prep_audio(audio, input_sr=44100, target_sr=16000, in_shape=16000):
    ...
    if input_sr != target_sr:
        audio = resample_audio(audio, input_sr, target_sr)
    if in_shape:
        audio = pad_or_trim_along_axis(audio, in_shape)
    return audio


def load_timbre_data(dataset_ids = ['Barthet2010', 
                                    'Grey1977', 
                                    'Grey1978', 
                                    'Iverson1993_Whole', 
                                    'Lakatos2000_Harm', 
                                    ...
                                    'Siedenburg2016_e3', 
                                    'Vahidi2020', 
                                    'Zacharakis2014_merged'],
                     input_sr=44100,
                     target_sr=44100):
    dissimilarities = {
        ds: load_dissimilarity_matrix(ds)
        for ds in dataset_ids
    }

    datasets = {
        ds: timbremetrics.get_audio(ds) 
        for ds in dataset_ids
    }
    audio = [
        {'dataset': ds, 'audio': prep_audio(item['audio'], input_sr, target_sr, target_sr), 'idx': idx}
        for ds, ds_items in datasets.items()
        for idx, item in enumerate(ds_items)
    ]
    # dataset to audio tensor
    ds_audio = {ds: [] for ds in datasets.keys()}

    for item in audio:
        ds_audio[item['dataset']].append(item['audio'])
    ds_audio = {ds: np.stack(values) for ds, values in ds_audio.items()}

    return dissimilarities, ds_audio 
...
def load_and_save_dissim_mat(sim_csv = 'jasmp/sim.csv', extended_csv: str = 'jasmp/extended_filelist.csv', out_path: str = 'jasmp'):
...
def df_attach_imt_pitch_dynamics(meta_path = '/homes/cv300/Documents/timbre-metric/jasmp/', seed_filelist='seed_filelist.csv'):
    ...
    df['dynamics'] = [get_sol_pitch_dynamics(get_fname(f))[1] for f in list(df['fpath'])]
    return df

def get_extended_files(SOL_PATH = '/import/c4dm-datasets/SOL_0.9_HQ/',
    ...
    ext_files = [(seed_imts[get_sol_IMT(os.path.basename(f))], f)
                 for f in get_sol_filepaths(SOL_PATH) 
                 if get_sol_IMT(os.path.basename(f)) in seed_imts.keys()]
    ext_files = list(filter(functools.partial(filter_fn, df, semitones=semitones), ext_files))

    df_ext_tmp = pd.DataFrame(ext_files, columns=['seed_id', 'filepath'])
    df_ext_tmp = df_ext_tmp[df_ext_tmp['seed_id'].isin(df.index)]
    df_ext_tmp['instrument'] = [get_sol_instrument(get_fname(f)) for f in list(df_ext_tmp['filepath'])]
    df_ext_tmp['IMT'] = [get_sol_IMT(get_fname(f)) for f in list(df_ext_tmp['filepath'])]
    df_ext_tmp['pitch'] = [get_sol_pitch_dynamics(get_fname(f))[0] for f in list(df_ext_tmp['filepath'])]
    df_ext_tmp['dynamics'] = [get_sol_pitch_dynamics(get_fname(f))[1] for f in list(df_ext_tmp['filepath'])]
    df_ext_tmp['instrument family'] = [inst_to_fam[pl.Path(f).parts[0]] for f in list(df_ext_tmp['filepath'])]
    return df_ext_tmp
...
def df_attach_meta(df, meta_path = '/homes/cv300/Documents/timbre-metric/jasmp/'):
    ...
    df['pitch'] = [get_sol_pitch_dynamics(get_fname(f))[0] for f in list(df['filepath'])]
    df['dynamics'] = [get_sol_pitch_dynamics(get_fname(f))[1] for f in list(df['filepath'])]
    df['instrument family'] = [inst_to_fam[pl.Path(f).parts[0]] for f in list(df['filepath'])]
    return df
